...
function FrogIcon({ className = "w-6 h-6" }: { className?: string }) {
  return (
    <svg viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      <path d="M18 62C18 42 30 22 50 22C70 22 82 42 82 62C82 78 68 86 50 86C32 86 18 78 18 62Z" fill="currentColor" opacity="0.85"/>
      <ellipse cx="35" cy="30" rx="13" ry="11" fill="currentColor"/>
      <ellipse cx="35" cy="29" rx="7" ry="6" fill="var(--color-frog-black, #0F1412)" opacity="0.9"/>
      <ellipse cx="37" cy="27" rx="2.5" ry="2" fill="white" opacity="0.7"/>
      <ellipse cx="65" cy="30" rx="13" ry="11" fill="currentColor"/>
      <ellipse cx="65" cy="29" rx="7" ry="6" fill="var(--color-frog-black, #0F1412)" opacity="0.9"/>
      <ellipse cx="67" cy="27" rx="2.5" ry="2" fill="white" opacity="0.7"/>
      <path d="M32 64Q50 76 68 64" stroke="var(--color-frog-black, #0F1412)" strokeWidth="2" strokeLinecap="round" fill="none" opacity="0.35"/>
    </svg>
  )
}
...
export function FrogAIChat() {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: "Something went wrong. Try again or message us on WhatsApp." }])
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 z-[90] w-14 h-14 rounded-full bg-frog-green text-black flex items-center justify-center shadow-[0_0_30px_rgba(122,201,67,0.3)] hover:shadow-[0_0_50px_rgba(122,201,67,0.4)] hover:scale-110 transition-all duration-300 group"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-[90] w-[380px] max-w-[calc(100vw-48px)] h-[520px] max-h-[calc(100vh-140px)] rounded-[28px] overflow-hidden flex flex-col border border-white/10 bg-frog-card/95"
            style={{ backdropFilter: 'blur(40px)' }}
          >
            {/* Header */}
            <div className="p-5 border-b border-white/5 flex items-center gap-3 shrink-0">
              <div className="w-9 h-9 rounded-full bg-frog-green flex items-center justify-center text-black">
                <FrogIcon className="w-5 h-5" />
              </div>
              <div>
                <h4 className="text-white font-black text-sm">FrogAI</h4>
                <p className="text-white/30 text-[10px] uppercase tracking-widest font-bold">Assistant</p>
              </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto frog-chat-scroll p-4 space-y-4">
              {messages.length === 0 && (
                <div className="text-center pt-8">
                  <div className="w-16 h-16 rounded-full bg-frog-green/10 mx-auto mb-4 flex items-center justify-center text-frog-green">
                    <FrogIcon className="w-9 h-9" />
                  </div>
                ...
                <div key={i} className={`flex gap-2.5 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-7 h-7 rounded-full shrink-0 flex items-center justify-center ${
                    ...
                    {msg.role === 'user' ? (
                      ...
                      <FrogIcon className="w-4 h-4 text-frog-green" />
                    )}
                  </div>
            ...
            <div className="p-4 border-t border-white/5 shrink-0">
              ...
              >
                <input
                  ...
                  aria-label="Type your message to FrogAI"
